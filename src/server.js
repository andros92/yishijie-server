...
async function findUserByFp(deviceFp, phoneFp) {
  // 手环即身份：设备指纹有效时只按设备指纹找账号，绝不用手机指纹兜底（防止双手环串号）
...
app.get('/api/yishijie/version', async (req, res) => {
  try {
    ...
    return json(res, 200, {
      success: true,
      versionCode: ver ? Number(ver.versionCode) || 3 : 3,
      versionName: (ver && ver.versionName) || '0.2.1',
...
app.post('/api/yishijie/admin/version', async (req, res) => {
  try {
    ...
    const ver = {
      ...
      versionName: String(versionName || '0.2.1'),
